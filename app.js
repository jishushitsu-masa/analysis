...
async function analyzeTest() {
    ...
    // Build Payload
    const parts = [
        { text: `あなたはプロの塾講師です。生徒（${grade}・科目：${subject}）のテスト結果画像を分析してください。
画像から問題と生徒の解答を読み取り、長文を避けて要点がパッと見てわかるように、以下のマークダウン形式で出力してください。

1. **全体への温かいアドバイス**（数行程度）
2. **分野別分析**（必ずテーブル形式で出力すること）
   | 分野・単元 | 予想得点率 | コメント・評価（ミスの傾向や今後の対策） |
   | :--- | :---: | :--- |
   ※ 画像から読み取れる範囲で、分野をいくつかピックアップしてテーブルにまとめてください。
3. **特に優先すべき弱点**（1〜2つ）` }
    ];
